...
public interface UserCourseMapper extends BaseMapper<UserCourse> {
        ...
        /** 批量设置课程所有成员的归档状态 */
        @Update("UPDATE user_course SET archived = #{archived} WHERE course_id = #{courseId}")
        int updateArchivedByCourseId(@Param("courseId") Long courseId, @Param("archived") Boolean archived);

        /** 检查课程是否有教师已归档 */
        @Select("SELECT COUNT(*) > 0 FROM user_course WHERE course_id = #{courseId} AND role = 'teacher' AND archived = 1")
        boolean existsTeacherArchived(@Param("courseId") Long courseId);
}
